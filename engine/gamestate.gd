...
func _player_connected(_id):
	...
	if not has_node("/root/Main"): # Game is in progress
		get_tree().get_root().add_child(main)
		get_tree().get_root().get_node("lobby").hide()
	else:
		_add_player(_id)	
	if get_tree().is_network_server():
		main.set_network_master(_id)

func _add_player(p_id):
	var player_scene = load("res://player/player.tscn")
	var player = player_scene.instance()
	var main = get_tree().get_root().get_node("Main")
	var spawn = main.get_node("spawn")
...
func host_game():
	...
	var err = host.listen(DEFAULT_PORT, supported_protocols, true);
	if err != OK:
...
func join_game():
	...
	host.connect_to_url('ws://localhost:8910/test/', PoolStringArray(), true); 
	get_tree().set_network_peer(host)
